...
def check_breakout(symbol, exchange, tv, mode_cfg, strength=2):
    """
    Generic breakout checker — works for ALL three scan modes.
    Uses mode_cfg to know which HTF/LTF intervals and bar counts to use.
    Exact same detection logic as original fractal_scanner.py.
    """
    ...
    result = {
        ...
        "exchange"          : exchange,
        "status"            : "OK",
        "alert"             : None,       # "HIGH_BREAK" | "LOW_BREAK" | None
        "direction"         : None,       # "BEARISH_SETUP" | "BULLISH_SETUP"
        "broken_level"      : None,
        "current_price"     : None,
        "candle_high"       : None,
        "candle_low"        : None,
        "candle_open"       : None,
        "candle_time"       : None,
        "sweep_type"        : None,       # "CLOSE_BREAK" | "WICK_SWEEP"
        "unmitigated_highs" : [],
        "unmitigated_lows"  : [],
    ...
    try:
        # ── HTF data  (30-min / 1-Week / 1-Day depending on mode)
        df_htf = tv.get_hist(
            symbol   = symbol,
            exchange = exchange,
            interval = mode_cfg["htf_interval"],
            n_bars   = mode_cfg["htf_bars"],
        )
        if df_htf is None or df_htf.empty:
            result["status"] = "ERROR"
            result["error"]  = f"No {mode_cfg['htf_label']} data"
            return result

        df_htf = df_htf[["open", "high", "low", "close", "volume"]].copy()

        # ── Unmitigated fractal levels from HTF
        u_highs, u_lows = get_unmitigated_levels(df_htf, strength)
        result["unmitigated_highs"] = u_highs
        result["unmitigated_lows"]  = u_lows

        # ── LTF data  (5-min / 1-Day / 1-Hour depending on mode)
        df_ltf = tv.get_hist(
            symbol   = symbol,
            exchange = exchange,
            interval = mode_cfg["ltf_interval"],
            n_bars   = mode_cfg["ltf_bars"],
        )
        if df_ltf is None or df_ltf.empty:
            result["status"] = "ERROR"
            result["error"]  = f"No {mode_cfg['ltf_label']} data"
            return result

        df_ltf = df_ltf[["open", "high", "low", "close", "volume"]].copy()
        ...
        # ── Check HIGH sweep  →  BEARISH setup
...
defaults = {
    "tv"           : None,
    "connected"    : False,
    "results"      : {},      # keyed by mode key: "default" | "weekly" | "daily"
    "last_scan"    : {},
    "scan_count"   : {},
    "custom_stocks": [],      # list of (SYMBOL, EXCHANGE) added by user
}
...
with st.sidebar:
    ...
    tv_user = st.text_input("Username", value="jarramuk69", placeholder="TV username (optional)")
    tv_pass = st.text_input("Password", value="Jarramukshadow#69", type="password", placeholder="TV password (optional)")

    if st.button("🔌  CONNECT TO TRADINGVIEW"):
        with st.spinner("Connecting..."):
            try:
                if tv_user and tv_pass:
                    st.session_state.tv        = TvDatafeed(tv_user, tv_pass)
                    st.session_state.connected = True
                    st.success(f"✅  Logged in as {tv_user}")
                else:
                    st.session_state.tv        = TvDatafeed()
                    st.session_state.connected = True
                    st.success("✅  Connected as Guest")
            except Exception as e:
...
with tab_scan:
    ...
    if scan_btn:
        ...
        for idx, (sym, exch) in enumerate(wl):
            ...
            r = check_breakout(sym, exch, tv, mode_cfg, strength)
            results.append(r)
            time.sleep(0.6)
